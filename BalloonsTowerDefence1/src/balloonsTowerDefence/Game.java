...
public class Game {
	...
	public Game(int[][] mapMatrix) {
		...
		this.balloonsInGame[0] = new BalloonRed(10, 11, grid);
	...
	public Game(FloorGrid grid) {
		...
		this.menuBg = LoadTexture("inGame_TowerPickerMenu");
		this.balloonsInGame = new Balloon[1];
		this.balloonsInGame[0] = new BalloonRed(0, 1, grid);
		this.roundManager = new RoundManager(balloonsInGame, 3, 3);
		this.player = new Player(grid, roundManager);
		this.player.initialize();
	...
	private void tickUI() {
		...
		gameUserInterface.drawString(pickTowerMenu.getX(), pickTowerMenu.getY() + 250, "Speed: x" + Timer.timeMultiplier);
